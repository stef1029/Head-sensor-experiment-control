...
def create_end_signal(output_path, source = None):
    """
    ...
    """
    if source:
        end_signal_path = os.path.join(output_path, f"end_signal_{source}.signal")
    else:
        end_signal_path = os.path.join(output_path, "end_signal.signal")
    # Create an empty file
    open(end_signal_path, 'w').close()

def check_for_signal_file(output_path, source = None):
    """
    ...
    if source:
        for file_name in os.listdir(output_path):
            if file_name == f"end_signal_{source}.signal":
                return True
        return False
    else:
        for file_name in os.listdir(output_path):
